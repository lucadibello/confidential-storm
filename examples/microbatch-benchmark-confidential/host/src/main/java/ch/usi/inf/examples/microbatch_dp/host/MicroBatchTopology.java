...
public class MicroBatchTopology {
    ...
    public static TopologyBuilder getTopologyBuilder() {
        ...
        builder
            ...
            .allGrouping(
                ComponentConstants.SPOUT.toString(),
                ComponentConstants.CONTROL_STREAM
            );
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (cliArgs.containsKey("mu")) System.setProperty(
            "dp.mu",
            cliArgs.get("mu")
        );
        if (cliArgs.containsKey("max-time-steps")) System.setProperty(
            "dp.max.time.steps",
            cliArgs.get("max-time-steps")
        );
        if (cliArgs.containsKey("parallelism")) System.setProperty(
            "synthetic.parallelism",
            cliArgs.get("parallelism")
        );
        if (testMode && !cliArgs.containsKey("parallelism")) System.setProperty(
            "synthetic.parallelism",
            "1"
        );

        int numUsers = cliArgs.containsKey("num-users")
            ? Integer.parseInt(cliArgs.get("num-users"))
            : Integer.getInteger("synthetic.num.users", 10_000_000);
        int numKeys = cliArgs.containsKey("num-keys")
            ? Integer.parseInt(cliArgs.get("num-keys"))
            : Integer.getInteger("synthetic.num.keys", 1_000_000);
        long seed = cliArgs.containsKey("seed")
            ? Long.parseLong(cliArgs.get("seed"))
            : Long.getLong("synthetic.seed", 42L);
        int runId = cliArgs.containsKey("run-id")
            ? Integer.parseInt(cliArgs.get("run-id"))
            : Integer.getInteger("synthetic.run.id", 1);
        String outputDir = cliArgs.getOrDefault(
            "output-dir",
            System.getProperty("synthetic.output.dir", "/logs/storm/profiler")
        );

        String sizesGbCsv = cliArgs.getOrDefault(
            "batch-sizes-gb",
            System.getProperty("microbatch.sizes.gb", "1,2,5")
        );
        int runsPerSize = cliArgs.containsKey("runs-per-size")
            ? Integer.parseInt(cliArgs.get("runs-per-size"))
            : Integer.getInteger(
                  "microbatch.runs.per.size",
                  MicroBatchConfig.DEFAULT_RUNS_PER_SIZE
              );
        long bytesPerTuple = cliArgs.containsKey("bytes-per-tuple")
            ? Long.parseLong(cliArgs.get("bytes-per-tuple"))
            : Long.getLong(
                  "microbatch.bytes.per.tuple",
                  MicroBatchConfig.DEFAULT_BYTES_PER_TUPLE
              );
        long completionTimeoutMs = cliArgs.containsKey("completion-timeout-ms")
            ? Long.parseLong(cliArgs.get("completion-timeout-ms"))
            : Long.getLong(
                  "microbatch.completion.timeout.ms",
                  MicroBatchConfig.DEFAULT_COMPLETION_TIMEOUT_MS
              );
        ...
        LOG.info(
            "Micro-batch: sizesGb={}, runsPerSize={}, bytesPerTuple={}, completionTimeoutMs={}",
            sizesGbCsv,
            runsPerSize,
            bytesPerTuple,
            completionTimeoutMs
        ...
        LOG.info(
            "Profiler: enabled={}, sampleRate={}, reportTicks={}, outputDir={}",
            ProfilerConfig.ENABLED,
            ProfilerConfig.SAMPLE_RATE,
            ProfilerConfig.REPORT_INTERVAL_TICKS,
            ProfilerConfig.OUTPUT_DIR
        );
        LOG.info("=============================================");
        ...
        conf.put(
            MicroBatchConfig.CONF_COMPLETION_TIMEOUT_MS,
            completionTimeoutMs
        );

        // Configure worker heap to match baseline (8 GB) to accommodate pre-generated batches.
