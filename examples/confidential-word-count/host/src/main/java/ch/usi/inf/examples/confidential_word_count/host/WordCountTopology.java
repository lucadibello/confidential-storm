...
public class WordCountTopology extends ConfigurableTopology {
    ...
    public int run(String[] args) {
        ...
        LOG.info(
            "Starting WordCountTopology in {} mode",
            isProd ? "PROD" : "LOCAL"
        );
        ...
        if (!isProd) {
            ...
            System.setProperty(
                "confidentialstorm.debug.exceptions.enabled",
                "false"
            );
            // Enable enclave service null-check proxy for local runs
            System.setProperty(
                "confidentialstorm.enclave.proxy.enable",
                "true"
            );
        }
        if (!isProd) {
    ...
    private boolean isProdEnvironment(String[] args) {
        if (args != null) {
            for (String arg : args) {
                if (
                    "--prod".equalsIgnoreCase(arg) ||
                    "--production".equalsIgnoreCase(arg)
                ) {
                    return true;
                }
