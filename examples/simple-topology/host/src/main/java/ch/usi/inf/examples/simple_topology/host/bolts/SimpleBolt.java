...
public class SimpleBolt extends ConfidentialBolt<SimpleEnclaveService> {
    ...
    @Override
    protected void processTuple(Tuple input, SimpleEnclaveService service) throws EnclaveServiceException {
        // extract `num` from tuple
        float num = input.getFloat(0);

        // send request to TEE
        LOG.debug("Sending request with number", num);
        SimpleEnclaveResponse response = service.query(new SimpleEnclaveRequest(
